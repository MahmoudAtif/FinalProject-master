...
class MainCubit extends Cubit<MainState> {
  ...
  Position ? position;
  void setUpMyLocation({required Position mPos}){
    this.position = mPos;
    emit(GetMyPostion());
  }
  ...
  void updateReport(int reportCode, String id ,String vio) {
    switch (reportCode) {
      case 10:
      ...
      case 20:
        ...
        break;
      case 21:
        if(vio == ConstantManger.Torok){
          _updateReport(newCode: 22, id: id);

        }else if(vio == ConstantManger.Bena2){
          _updateReport(newCode: 24, id: id);
        }
        else if(vio == ConstantManger.Qemama){
          _updateReport(newCode: 23, id: id);
        }
        else if(vio == ConstantManger.Enara){
...
*/
  ...
  void _updateReport({required int newCode, required String id}) {
    ...
    FirebaseFirestore.instance
        .collection(ConstantManger.Reports)
        .doc(id)
        .update({'reportCode': newCode}).then((value) {
      getHistoryReport();
    });
  }
